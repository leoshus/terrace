...
import com.sdw.soft.core.annotation.EntityAutoCode;
import com.sdw.soft.core.annotation.MetaData;
import com.sdw.soft.core.entity.AncestorEntity;
import com.sdw.soft.core.exception.TerraceException;
...
import com.sdw.soft.core.utils.ExtStringUtils;
import com.sdw.soft.core.view.OperationResult;
 ...
 * @version 1.0.0
 */
public abstract class AncestorController <T extends AncestorEntity<ID>,ID extends Serializable> extends RestActionSupport 
						implements Preparable,ModelDriven<Object>{

	private static final long serialVersionUID = -4326741399920283097L;

	private final Logger logger = LoggerFactory.getLogger(AncestorController.class);
	
	private static final String PARAM_NAME_FOR_AUTOCOMPLETE = "term";//Autocomplete 组件传递的查询参数
	/*根据请求参数指定跳转到特定的JSP页面,如有相同的处理逻辑，但是不同的业务需求按照不同页面显示则可以指定此参数跳转到指定的JSP页面*/
	protected static final String PARAM_NAME_FOR_FORWARD_TO = "_to_";
	/*分页查询方法特定的数据处理格式标识参数,默认标识返回查询JSON数据,可指定如xls标识导出相应的(不分页)查询数据*/
	protected static final String PARAM_NAME_FOR_EXPORT_FORMAT = "_format_";
	/*子类指定特定泛型对应的实体Service接口对象*/
	...
	public String getActionName(){
		//FIXME 后期考虑优化为直接注入Struts的ActionNameBuilder方式
	...
	public AncestorController(){
		...
		try {
			...
			throw new TerraceException(e.getMessage(),e);
		}
	}
	
	/**
	 * 重写Preparable中的prepare方法
	...
	protected String getRequiredParameter(String name){
		...
		if(StringUtils.isBlank(value)){
			throw new TerraceException("web.param.disallow.empty:" + name);
		}
		return value;
	}
	/**
	 * 获取参数对应的值，当参数为空时返回默认值defaultValue
	 * @param name
	 * @param defaultValue
	 * @return
	 */
	...
	/**
	 * 通用forward跳转方法，根据to跳转到对应的JSP页面，如果to参数为空，则抛出必要参数缺失异常
	 * @return
	 */
	...
	/**
	 * 用于创建或更新时跳转到通用的Tabs页面
	 * 一般创建时tabs页面只有第一个tab可编辑，其余为disabled状态
	 * @return
	 */
	 ...
	 * 如果子类需要根据其他如code代码的等属性查看数据，则根据业务逻辑覆写此方法即可
	 */
	...
	/**
	 * doCreate调用之前的prepare接口自动回调方法
	 * 准备new实体对象以备ParametersInterceptor进行参数绑定 FIXME
	 */
	public void prepareDoCreate(){
		newBindingEntity();
	}
	/**
	 * 为了避免由于权限配置不严格，导致未授权的Controller数据操作访问，父类提供protected基础实现，子类根据需要覆写public然后调用此基类方法
	 * @return
	 */
	@MetaData(title = "创建")
	protected HttpHeaders doCreate(){
		...
		checkEntityAclPermission(bindingEntity);
		//FIXME revison
//		ExtRevisonListener.setOperationEvent(RevisionType.ADD.name());
    ...
    public HttpHeaders checkUnique() {
        ...
        if (entities == null || entities.size() == 0) {// 未查到重复数据
            ...
            if (entities.size() == 1) {// 查询到一条重复数据
                ...
                this.setModel(Boolean.FALSE);
                throw new TerraceException("error.check.unique.duplicate: " + element + "=" + value);
            }
        }
        return buildDefaultHttpHeaders();
    }
  ...
  //FIXME revison
   /* @MetaData(title = "版本数据对比")
    ...
    protected String convertPropertyDisplay(Object entity, Field field, Object value) {
        ...
        if (value instanceof AncestorEntity) {
            @SuppressWarnings("rawtypes")
            AncestorEntity ancestorEntity = (AncestorEntity) value;
            String label = "N/A";
            try {
                label = ancestorEntity.getDisplayLabel();
                ...
                try {
                    ...
                    AncestorEntity auditTargetEntity = (AncestorEntity) obj;
